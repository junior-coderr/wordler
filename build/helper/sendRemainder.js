...
const getMeaning = (words) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const geminiKey = process.env.GEMINI_KEY;
    const prompt = `Provide the definitions and example sentences for each word in this list: [${words.join(
      ", "
    )}]. Format each entry exactly like this: "1. word: definition. Example: example sentence." Ensure there are no symbols, including asterisks or any other formatting. For example:
  ...
  Now provide definitions and examples for each word in the list in the same format, starting from 1.`;
    try {
      ...
      console.log("error in getMeaning", error);
      return null;
    }
  });
const sendRemainder = () =>
